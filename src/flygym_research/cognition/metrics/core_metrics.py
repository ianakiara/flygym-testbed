...
def history_dependence(transitions: list[StepTransition]) -> dict[str, float]:
    ...
    for transition in transitions:
        ...
        # Quantize coarse target direction to compare actions under similar current state
        # while still allowing different action histories to accumulate within a bucket.
